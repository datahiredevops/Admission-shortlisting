...
import { 
  ...
  CheckCircle, XCircle, AlertCircle, Clock, AlertTriangle, BookOpen, Users 
} from "lucide-react";
...
export default function StudentDashboard() {
  ...
  const handleOfferResponse = async (appId: string, action: "Accept" | "Decline", appName: string) => {
    const message = action === "Accept" ? `🎉 Accept offer for ${appName}?` : `⚠️ Decline offer for ${appName}?`;
    if (!confirm(message)) return;
    try {
      await api.post(`/student/respond-offer/?application_id=${appId}`, { action });
                                ...
                                <div className="bg-gray-50 p-4 border-t border-gray-100">
                                    {app.application_status === "Accepted" ? (
                                        <div className="flex gap-2">
                                            <button onClick={() => handleOfferResponse(app.application_ref_id, "Accept", app.institution_name)} className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded shadow text-sm">Accept</button>
                                            <button onClick={() => handleOfferResponse(app.application_ref_id, "Decline", app.institution_name)} className="flex-1 bg-white border border-red-200 text-red-600 hover:bg-red-50 font-bold py-2 rounded text-sm">Decline</button>
                                        </div>
                                    ) : (
        ...
        {activeTab === "application" && selectedApp && (
             <div className="max-w-4xl mx-auto space-y-6 animate-in slide-in-from-right-4 duration-300">
                ...
                {selectedApp.application_status === "Accepted" && (
                    <div className="bg-green-50 border border-green-200 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                        <div className="flex items-center gap-4"><div className="bg-green-100 p-3 rounded-full"><CheckCircle className="text-green-600 w-8 h-8" /></div><div><h2 className="text-xl font-bold text-green-800">Admission Offered!</h2><p className="text-green-700 text-sm">You have been selected. Please accept to confirm your seat.</p></div></div>
                        <div className="flex gap-3"><button onClick={() => handleOfferResponse(selectedApp.application_ref_id, "Accept", selectedApp.institution_name)} className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-bold shadow-lg">Accept Offer</button><button onClick={() => handleOfferResponse(selectedApp.application_ref_id, "Decline", selectedApp.institution_name)} className="bg-white border border-red-200 text-red-600 hover:bg-red-50 px-6 py-2 rounded-lg font-bold">Decline</button></div>
                    </div>
                )}
                {selectedApp.application_status === "Waitlisted" && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 flex items-center gap-4 shadow-sm text-yellow-800">
                        <div className="bg-yellow-100 p-3 rounded-full"><AlertTriangle className="text-yellow-600 w-8 h-8" /></div>
                        <div><h2 className="text-xl font-bold">You are on the Waitlist</h2><p className="text-sm mt-1 opacity-90">We will notify you immediately if a seat becomes available.</p></div>
                    </div>
                )}
                ...
                {selectedApp.application_status === "Offer Accepted" && (
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-3 text-blue-800"><CheckCircle size={20} /> <span className="font-medium">✅ You have accepted this offer! Please check email for fee payment.</span></div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Choices & Quota (UPDATED) */}
                    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                            <AlertCircle size={18} className="text-green-600"/> Choices & Quota
                        </h3>
                        <div className="space-y-3 text-sm">
                            ...
                            {/* --- NEW SECTION: COMMUNITY & QUOTA --- */}
                            <div className="grid grid-cols-2 gap-4 pt-2 mt-2 border-t">
                                <div>
                                    <span className="block text-gray-500 text-xs uppercase">Community</span>
                                    <span className="font-bold text-gray-800">{selectedApp.community}</span>
                                </div>
                                <div>
                                    <span className="block text-gray-500 text-xs uppercase">Admission Quota</span>
                                    {/* Logic to show Merit vs Management */}
                                    <span className={`font-bold ${selectedApp.is_management_quota ? 'text-orange-600' : 'text-green-600'}`}>
                                        {selectedApp.is_management_quota ? "Management Quota" : "Merit / General"}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                {/* Parent Info */}
                <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                    <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Users size={18} className="text-purple-600"/> Parent / Guardian Info</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                         <div><span className="block text-gray-500 text-xs uppercase">Father's Name</span><span className="font-medium text-lg">{selectedApp.father_name || "N/A"}</span></div>
                         <div><span className="block text-gray-500 text-xs uppercase">Parent Contact</span><span className="font-medium text-lg">{selectedApp.father_mobile || "N/A"}</span></div>
                    </div>
                </div>
